...
This guide will assist you with upgrading from jQuery Mobile 1.3 to 1.4. It also prepares you for 1.5 by informing you about deprecated features. See the [changelog](https://jquerymobile.com/changelog/1.4.0/) for a list of all changes and new features.


## Supported platforms

([60aa05b](https://github.com/jquery/jquery-mobile/commit/60aa05b813488beff1d8c9bf8a0715b93ee7deed)) In previous versions A-grade support was based on whether a browser supports media queries and added Internet Explorer 7 and 8. As from 1.4 we also test for pseudo element support, because we use `:after` for icons. This means Internet Explorer 7 and Blackberry OS 5 are now C-grade.
...
([8abb930](https://github.com/jquery/jquery-mobile/commit/8abb930ffefca335f4adf8dc743b6bbd5df47a7f)) The default theme has been replaced and the new theme has two swatches, A (light) and B (dark). To keep using similar colors as the previous default theme you have to change the value you set for the `theme` option (`data-theme="*"`).
...
The blue and yellow swatches didn't return in the new default theme. You can use the [ThemeRoller](https://themeroller.jquerymobile.com) to create a complete custom theme or to add swatches to the default theme.

([abf1d0f](https://github.com/jquery/jquery-mobile/commit/abf1d0f19ac3a919d93193ff4422286d83cfd0b9)) For convenience we updated the 1.3 default theme to 1.4 and added it as ["classic" theme](https://demos.jquerymobile.com/1.4.0/theme-classic/) to the demos.

### Custom theme

When upgrading to 1.4 you have to replace your custom theme file. The easiest way to upgrade your 1.3 custom theme is by uploading it in the [ThemeRoller](https://themeroller.jquerymobile.com) and download it as 1.4 theme.
...
([#3750](https://github.com/jquery/jquery-mobile/issues/3750), [3cc45ef](https://github.com/jquery/jquery-mobile/commit/3cc45efdbde0ce9c63f40544a779014017d48c92)) The active and focus state used to use one global style, but they are now themeable. When you use the ThemeRoller to upgrade your 1.3 custom theme, it will initially use the 1.3 global active and focus color for all swatches. You can customize this for each swatch.
...
* The level of [specificity](https://www.w3.org/TR/css3-selectors/#specificity) of many selectors has been increased due to a switch from JS to CSS for theme inheritance
...
([98b90c3](https://github.com/jquery/jquery-mobile/commit/98b90c3f03bb30b126b2d0b9079e9ab920e0d301)) We changed the default for option `theme` in some widgets. The general rule is now that all themeable components inherit the theme swatch from their container. This is done via CSS. If there is no container for which a theme has been specified, the page theme will be used. This now defaults to theme A. Because widgets don’t have a default theme specified anymore, you must set option theme if you use widgets outside a page.
...
([c747016](https://github.com/jquery/jquery-mobile/commit/c7470167dbdd5577bb1487b77fdb30166bcfdd55)) Button styled elements used to get enhanced by the `buttonMarkup` method, which generated the `ui-btn-inner` and `ui-btn-text` elements. In 1.4 those inner elements are no longer generated and there are no calls to buttonMarkup() from other widgets anymore.
...
([efc1f51](https://github.com/jquery/jquery-mobile/commit/efc1f5186276f5a922df3ee53bc9cafbc4b3533f)) If you are using custom icons you have to change the selector in your icon background-image rules because we now use the `:after` pseudo element instead of generating a span with the icon class.
...
([173a101](https://github.com/jquery/jquery-mobile/commit/173a1012a9649e2407220c2cbda64ce91aa1d623)) Instead of adding `data-rel="dialog"` to the page anchor link, set page option `dialog` to `true`. You can do this by adding `data-dialog="true"` to the page that you want to be styled as dialog.
...
([c51fe84](https://github.com/jquery/jquery-mobile/commit/d333e8c52bd044a31bde2fdf3b559c679c51fe84)) Headers and footers are now using the new Toolbar widget. When you are using the "add back button" feature, all related `data-` attributes (`data-add-back-btn`, etc.) should be added to the header element, instead of the page.
...
([#5271](https://github.com/jquery/jquery-mobile/issues/5271), [fe60da4](https://github.com/jquery/jquery-mobile/commit/fe60da49b06f42e66a648adbd69f6dff349b4392)) .trigger( "expand|collapse" ) no longer works. Use the new `expand` and `collapse` methods instead.
...
([63dd22f](https://github.com/jquery/jquery-mobile/commit/63dd22f365077ffa45e7910f7a360bd70df9ed07)) The content of a collapsible now inherits the theme from a parent container by default. Set `contentTheme` to `false` (`data-content-theme="false"`) to have a collapsible without content theme.
...
([3650c85](https://github.com/jquery/jquery-mobile/commit/3650c85cecd63f81165969396951ef0c2db24fe6)) `data-role="fieldcontain"` has been deprecated. Add class `ui-field-contain` instead.
...
([ed89730](https://github.com/jquery/jquery-mobile/commit/ed89730f42ad83cad7e8006a1ecd33ef64738192), [#6321](https://github.com/jquery/jquery-mobile/issues/6321), [0479daa](https://github.com/jquery/jquery-mobile/commit/0479daa0e3d358153a455d5448d395e67d9a1472)) The auto-detection of images in list items has been deprecated to improve performance and small 16x16 images on the left (`ui-li-icon`) has been deprecated as feature.
