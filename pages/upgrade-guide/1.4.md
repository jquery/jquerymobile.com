...
The default theme has been replaced and the new theme has two swatches, A (light) and B (dark). To keep using similar colors as the previous default theme you have to change the value you set for the `theme` option (`data-theme="*"`).
...
Headers and footers are now using the new Toolbar widget. When you are using the "add back button" feature, all related `data-` attributes (`data-add-back-btn`, etc.) should be added to the header element, instead of the page.
